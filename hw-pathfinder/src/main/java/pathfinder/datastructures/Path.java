...
public class Path<P> implements Iterable<Path<P>.Segment> {
    ...
    //          path.get(0) is the first step from the start object to an intermediate objects, and
    //          path.get(path.size() - 1) is the final step from an intermediate object to the end
    //      total cost along the path => cost
    //      the destination point in this path, opposite the start object => getEnd()
    ...
    public Iterator<Path<P>.Segment> iterator() {
        ...
        return new Iterator<Path<P>.Segment>() {
